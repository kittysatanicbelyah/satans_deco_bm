...
@Override
    ...
    //inputs manipulation
//default bars. Don't think much about textures. They're set in DecoBlockModelProvider currently
    private void barsBlockState(DeferredBlock<Block> block) {
        DeferredBlock<Block> wax = block;
    ...
    barsBlockInternal(wax.get(), wax.getId().toString(),
                ResourceLocation.fromNamespaceAndPath(satans_deco_bm.MODID, "block/" + block.getId().getPath()),
                ResourceLocation.fromNamespaceAndPath(satans_deco_bm.MODID, "block/" + block.getId().getPath()),
                ResourceLocation.fromNamespaceAndPath(mId, blockId));
    ...
    private void barsNoCapBlockState(DeferredBlock<Block> block) {
        DeferredBlock<Block> wax = block;
        if (block.getId().getPath().contains("waxed_")) block = BMSupGen.getDecoNonWaxed(block);
        String blockId = BMSupGen.BlockIdFilter(block);
        String mId = BMSupGen.ModIdFilter(block);
    ...
    private void barsNonstraightBlockState(DeferredBlock<Block> block) {
        DeferredBlock<Block> wax = block;
        if (block.getId().getPath().contains("waxed_")) block = BMSupGen.getDecoNonWaxed(block);
        String blockId = BMSupGen.BlockIdFilter(block);
        String mId = BMSupGen.ModIdFilter(block);
    ...
    private void barsNonstraightFalseEndsBlockState(DeferredBlock<Block> block) {
    ...
    private void barsVBlockState(DeferredBlock<Block> block) {
    ...
    private void barsUBlockState(DeferredBlock<Block> block) {
    ...
    private void barsUNoCapBlockState(DeferredBlock<Block> block) {
    ...
    private void barsReverseVBlockState(DeferredBlock<Block> block) {
    ...
    private void barsReverseUBlockState(DeferredBlock<Block> block) {
    ...
    private void barsReverseUNoCapBlockState(DeferredBlock<Block> block) {
    ...
    private void panelBlockState(DeferredBlock<Block> block) {
        DeferredBlock<Block> wax = block;
        if (block.getId().getPath().contains("waxed_")) block = BMSupGen.getDecoNonWaxed(block);
        panelBlockInternal(wax.get(), wax.getId().toString(),
                ResourceLocation.fromNamespaceAndPath(satans_deco_bm.MODID, "block/" + block.getId().getPath().replace("l", "")),
                ResourceLocation.fromNamespaceAndPath(satans_deco_bm.MODID, "block/" + block.getId().getPath().replace("l", "")));
    }
